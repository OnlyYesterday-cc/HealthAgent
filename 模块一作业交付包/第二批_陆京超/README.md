...
本目录分批完善。目前已完成TC-001至TC-015的清单、运行入口与功能验证，以及D01/D03专项核验；完整30条整合验证和报告补充留待后续批次。
...
入口另支持defects（共享3条专项）、module1（整合30条）、security（安全模块6条）及all。本目录已归档functional、defects和security结果，module1及all尚未在本轮执行。15条已包含在整合30条中，不累加为45条。
...
- 执行结果：分批保存的functional、defects、security日志、JUnit XML和环境信息。
